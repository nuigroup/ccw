...
#ifndef TARGET_OSX // because these won't work
...
void ccwServer::startMySQLServer() 
{
	...
	if(!system("start .\\data\\plugins\\MySQL\\bin\\mysqld.exe --standalone --console"))
	{
		mySQLIsRunning = true;
		cout << "[MySQL Control] MySQL daemon bin/mysqld.exe called" << endl;
	}
	else
	{
		mySQLIsRunning = false;
...
#ifdef _WIN32
	if(!system(".\\data\\plugins\\MySQL\\bin\\mysqladmin -u root -p shutdown"))
	{
		b_mySQL->setValue(false);
		mySQLIsRunning = false;
		cout << "[MySQL Control] MySQL admin shutdown called" << endl;
	}
	else
	{
		cout << "[ERROR] Could not stop MySQL daemon." << endl;
