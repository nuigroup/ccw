...
ofxHTTPServer::ofxHTTPServer() {
	...
	maxClients = 10;
	numClients = 0;
	uploadDir = ofToDataPath("/data/uploads/",false);
}
...
int ofxHTTPServer::answer_to_connection(void *cls,
	...
	// bugfix - this is the root, subfolders??
	if (strlen(url) == 1) {
		url = instance.localIndex.c_str();
	}
	...
	if(NULL == *con_cls){
		...
		if (instance.numClients >= instance.maxClients){
			fs_file file;
			file = getFileContent("error/503.html");
			if(!file.buffer){
	...
	// if the extension of the url is any other try to serve a file
		...
		cout << method << " serving from filesystem: " << instance.fsRoot << url << endl;
		
		fs_file file;
		file = getFileContent(instance.fsRoot + url);

		if(!file.buffer) { // file couldn't be opened
			cerr << "USER ERROR: file could not be opened trying to serve 404.html" << endl;
			file = getFileContent(ofToDataPath("error/404.html", true));
			if(!file.buffer){
				file.buffer = new char[100];
				strcpy(file.buffer, "404");
				file.length = strlen(file.buffer);
			}
...
void ofxHTTPServer::setIndex(string index){
	localIndex = index;
}
