...
void ccwConfigResponse::httpGet(string url) {
	...
	responseStr.append(ofToString(int(ccwServer::instance()->httpSettings.port)));
	...
	responseStr.append("</b></li><li>The idle time (before closing the connection) is <b>");
	responseStr.append(ofToString(ccwServer::instance()->httpSettings.idleTime));
	
	responseStr.append(" seconds</b></li><li>When \'Debug Logging\' is selected, logs will be written to <b>");
